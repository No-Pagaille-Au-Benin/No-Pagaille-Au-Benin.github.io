...
const pagailleur = await client.service('pagailleur').find({
    query: {
      $sort: { createdAt: -1 },
      $limit: 1
    }
  ...
  console.log(pagailleur);

  document.querySelector('#onlinecount').innerHTML = pagailleur.state;
};


// We will also see when new users get created in real-time
client.service('pagailleur').on('created', function(pagailleur){
        var state = pagailleur.state;
        console.log("Receive pagailleur");
        console.log(pagailleur)
        document.querySelector('#onlinecount').innerHTML = state;
    
});

// client.service('pagailleur').create({
// }).then(function (p) {
//     console.log("Pagailleur is created");
// }).catch(function (err) {
